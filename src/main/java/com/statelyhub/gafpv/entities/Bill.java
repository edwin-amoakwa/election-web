...
@Entity
@Table(name = "bills")
public class Bill extends DateRecord3 {
...
//    }
    ...
    public GeneralLedger getExpnse() {
        GeneralLedger ledger = new GeneralLedger();
        ledger.setBatch(getId());
        ledger.setAmount(getAmountPayable());
        ledger.setUnit(getUnit());
        ledger.setDebit(-getAmountPayable());
        ledger.setDebitCredit(DebitCredit.DEBIT);
        ledger.setAccount(getExpenseAccount());
        ledger.setValueDate(getValueDate());
        ledger.setNarration(paymentDetail);
//        ledger.set
        return ledger;
    }

    public GeneralLedger getPayble() {
        GeneralLedger ledger = new GeneralLedger();
        ledger.setBatch(getId());
        ledger.setAmount(getAmountPayable());
        ledger.setUnit(getUnit());
        ledger.setCredit(getAmountPayable());
